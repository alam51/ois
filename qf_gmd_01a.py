...
class Equipment(DbConnector):
    def __init__(self, ss_id, time_start, time_end):
        ...
        self.equipment_voltage_id_name_type_df2 = self.equipment_voltage_id_name_type_df1. \
            sort_values(by=['voltage_level', 'type'], ignore_index=True)
        ...
        for i in self.output_df.index:
            for j in self.output_df.columns:
                try:
                    if j[4] == 'voltage':
                        self.output_df.loc[i, j] = self.voltage_df[self.voltage_df['equipment_id'] == j[3]].loc[i, 'value']
                    elif j[4] == 'mega_watt':
                    ...
                    self.output_df.loc[i, j] = pd.NA
        b = 4


t1 = datetime.datetime.now()
equipment = Equipment(21, '2021-8-11 18:01', '2021-8-11 19:31')
